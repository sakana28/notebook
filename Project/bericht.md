...
Um die Übertragung großer Datenmengen zwischen PS und PL zu implementieren, habe ich die AXI-DMA und AXI-FIFO IP Cores von Xilinx sowie das AXI4-Stream Protokoll gelernt. Ich habe gelernt, wie diese für die Datenübertragung zwischen dem Prozessor und der programmierbaren Logikschicht des Systems verwendet werden können. Anschließend implementierte ich eine Bildverarbeitungsanwendung, die jeden dieser beiden IP-Kerne verwendet. Sie implementiert die Funktion, ein Bild von der SD-Karte zu lesen, es in PL zwischenzuspeichern, es dann zurück in PS zu übertragen und es schließlich wieder auf die SD-Karte zu schreiben.

Eine meiner Prioritäten war es, die Unterschiede zwischen den beiden IP-Cores zu verstehen und zu analysieren, welcher für mein Projekt besser geeignet ist. Durch die Verwendung des AXI-DMA-IP-Cores war ich in der Lage, die Übertragung von Informationen vom DDR-Speicher des PS zum PL direkt und ohne Beteiligung des Prozessors zu implementieren. Der Prozessor kann während des Informationstransfers andere Operationen ausführen, ohne dass ein ständiges Polling, d.h. eine Abfrage, erforderlich ist.