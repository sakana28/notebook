...
Ein weiteres Problem, das ich bei diesem Projekt hatte, war die Verwendung der UART-Schnittstelle. Ich hatte Schwierigkeiten, die UART-Schnittstelle auf meinem Ubuntu-System zu finden und zu konfigurieren, um sie in meinem Programm zu verwenden. Nach der Suche in der Web-Community wusste ich, dass ich den richtigen Namen des USB-Geräts im Verzeichnis /dev finden und es dann im Minicom-Terminalprogramm als serielle Schnittstelle konfigurieren musste. Nach diesem Vorgang könnte auch ein Terminalprogramm außerhalb von Minicom, wie z.B. das in Vitis eingebaute serielle Terminal, die gewünschte serielle Schnittstelle finden.

Die aufgetretenen Probleme haben mich gelehrt, dass es eine Vielzahl von komplexen Fehlern gibt, die bei der Verwendung von eingebetteten Systemen auftreten können und die mit verschiedenen Faktoren wie der Entwicklungsumgebung und den Hardwaregeräten zusammenhängen. Um die verschiedenen Herausforderungen zu bewältigen und die Probleme zu lösen, die bei der Arbeit mit diesen Systemen auftreten können, ist es daher notwendig, meine Fähigkeiten im Umgang mit Linux-Systemen und -Tools zu verbessern und die Community und die offizielle Dokumentation gut zu nutzen.
### Fall 2
...
## HDMI
Nach der erfolgreichen Implementierung der Übertragung und Zwischenspeicherung großer Datenmengen auf der PS- und PL-Seite bestand meine nächste Aufgabe darin, zu lernen, wie man einen HDMI-Ausgangspfad für die Anzeige von Bildern auf einem Monitor erstellt. Um den HDMI-Ausgang zu implementieren, verwendet das Zedboard-Entwicklungsboard einen externen Codec, den ADV7511 von ADI. Dieser Codec bietet eine I2C-Schnittstelle, über die er vom Benutzer konfiguriert werden kann. Daher muss auf der PL-Seite der IP-Core der I2C-Schnittstelle des Zedboards verwendet werden.

Der HDMI-Ausgangspfad besteht aus folgenden IP-Kernen: dem VDMA-Kern, dem Video Timing Controller IP-Kern (VTC), dem AXI-VDMA IP-Kern und dem Video Output IP-Kern (Vid_Out). Der AXI-VDMA-IP-Core hat ähnliche Funktionen wie der AXI-DMA-Core, bietet aber zusätzlich eine Schnittstelle zu den videorelevanten Synchronisationssignalen und Frame-Buffer-Funktionen. Der VTC-IP-Core kann als Timer betrachtet werden, der die für die Ausgangsüberwachung notwendigen Timing-Impulse erzeugt. Der Vid_Out-Kern wandelt den als AXI4-Stream-Protokoll übertragenen Datenstrom in Videodaten zur Ansteuerung eines externen Videoempfängers (z.B. eines Monitors) um und synchronisiert die Videodaten mit den vom VTC bereitgestellten Timing-Signalen.
