...
## Linux
Nachdem ich meine Hauptaufgabe, die Implementierung von Bildverarbeitungsfunktionen, durch die Entwicklung der Hardware auf der PL-Seite und des BareMetal-Programms auf der PS-Seite abgeschlossen habe, konnte ich einige Erfahrungen sammeln. Als nächstes habe ich meine Kenntnisse erweitert, um Linux auf dem Zynq zu untersuchen. Dazu verwendete ich das PetaLinux-Tool, das eine einfache, schnelle und effiziente Möglichkeit bietet, ein Embedded Linux-Image für den Zynq-Prozessor zu erstellen und anzupassen.
Mit PetaLinux konnte ich die Komponenten auswählen, die ich für mein Projekt benötigte, darunter den Kernel, den Bootloader, die Systembibliotheken und die Anwendungen. Diese Tools führten mich durch den Prozess der Image-Erstellung und ermöglichten es mir, das Linux-System auf meine Hardware zu portieren.
Ich habe mit einfachen Befehlen ein PetaLinux-Projekt auf der von Vivado generierten Hardware-Plattform erstellt und konfiguriert. Das Zedboard wurde dann über eine SD-Karte gebootet, die in zwei Sektoren aufgeteilt war, einen für das Linux-Image und den anderen für den Systemspeicherbereich. Unter embedded Linux habe ich die SD-Karte gemountet und das einfache Programm im Speicherbereich mit dem in rootfs enthaltenen gcc kompiliert und ausgeführt.
Obwohl dieses Wissen nicht zur Verbesserung meines Projekts genutzt wurde, war das Erlernen von Linux auf dem Zynq dennoch eine wertvolle Erfahrung und vermittelte mir ein tieferes Verständnis dafür, wie Software und Hardware in eingebetteten Systemen zusammenwirken. Diese Fähigkeiten werden mir bei der Entwicklung komplexerer Projekte in Zukunft von großem Nutzen sein.